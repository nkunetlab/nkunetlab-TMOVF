...
import torch
from dataset.new_imagenet import get_new_imagenet_small_testset
import numpy as np
import random
...
testloader = get_new_imagenet_small_testset()
...
def tocsv(name, i, prefix):
    list0 = name.tolist()
    df = pd.DataFrame(data=list0)
    df.to_csv('./new_imagenet_results/COF_testset' + prefix[1:] + f'/list{i}.csv', mode="a", encoding="utf_8_sig", header=1, index=0)
