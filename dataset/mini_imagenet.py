...
def get_mini_imagenet(size=[224, 224], batch_size_train=64, batch_size_test=1000):
    transform = transforms.Compose(
        [transforms.Resize(size),
         transforms.ToTensor(),
         transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    train_dataset = ImageFolder(root="./data/mini_imagenet/train", transform=transform)
    test_dataset = ImageFolder(root="./data/mini_imagenet/test", transform=transform)
...
def get_mini_imagenetnotShuffle(size=[224, 224], batch_size_train=64, batch_size_test=1000):
    transform = transforms.Compose(
        [transforms.Resize(size),
         transforms.ToTensor(),
         transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
...
def get_mini_imagenet2(size=[224, 224], batch_size_test_train=64, batch_size_test_test=1000):
    # can't change 'batch_size_test=1000'
    _, test_loader = get_mini_imagenet(size=size, batch_size_test=1000)
    test_loader_train_x = []
    test_loader_train_y = []
    test_loader_test_x = []
    test_loader_test_y = []
...
def get_mini_imagenet_small_trainset(size=[224, 224], batch_size_train=100):
    train_loader, _ = get_mini_imagenetnotShuffle(size=size, batch_size_train=1000)
    train_x = []
    train_y = []
...
def get_mini_imagenet_small_testset(size=[224, 224], batch_size_test=100):
    _, test_loader = get_mini_imagenetnotShuffle(size=size, batch_size_test=1000)
    test_x = []
    test_y = []
    for idx, (i, j) in tqdm(enumerate(test_loader)):
        if idx == 5:
