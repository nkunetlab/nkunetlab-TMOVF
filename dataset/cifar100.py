...
'''
CIFAR100数据集有100个类,每个类有600张大小为32×32的彩色图像，其中500张作为训练集，100张作为测试集，对于每一张图像，它有fine_labels和coarse_labels两个标签，但实际代码结果中只有一个标签，范围是0-99
'''


def getCifar100(size=[224, 224], batch_size_train=64, batch_size_test=1000):
...
def getCifar100notShuffle(size=[224, 224], batch_size_train=64, batch_size_test=1000):
    transform = transforms.Compose(
        [transforms.Resize(size),
         transforms.ToTensor(),
         transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
...
def getCifar1002(size=[224, 224], batch_size_test_train=64, batch_size_test_test=1000):
    # can't change 'batch_size_test=1000'
    _, test_loader = getCifar100(size=size, batch_size_test=1000)
    test_loader_train_x = []
    test_loader_train_y = []
    test_loader_test_x = []
    test_loader_test_y = []
...
def get_cifar100_small_trainset(size=[224, 224], batch_size_train=100):
    train_loader, _ = getCifar100notShuffle(size=size, batch_size_train=1000)
    train_x = []
    train_y = []
...
def get_cifar100_small_testset(size=[224, 224], batch_size_test=100):
    _, test_loader = getCifar100notShuffle(size=size, batch_size_test=1000)
    test_x = []
    test_y = []
    for idx, (i, j) in tqdm(enumerate(test_loader)):
        if idx == 5:
